...
const AirlineTicketsGeneralInformation = () => {
  ...
  const [redeemedValue, setRedeemedValue] = useState("Domestic");
  const handleChangeone = (event) => {
    setRedeemedValue(event.target.value);
  };
    ...
    await axios
      ...
      .then((response) => {
        ...
        if (response?.data?.ProductsVariantions?.length > 0) {
          ...
          setRedeemedValue(response?.data?.redeemedValue)
    ...
    const fetchData = async () => {
      try {
        const response = await axios
          .get(`airfeature/Get_airline_feature`, {
            withCredentials: true,
          })
          .then((res) => {
            fetchHsnCode(res.data?.ProductSubCategory);
  ...
  useEffect(() => {
    checkButtonState()
  }, [items, fields, tags, fromLocation, destinationLocation]);

  const checkButtonState = () => {

    if (items.length >= 5 && fields.length > 0 && tags.length > 0) {
      if (localStorage.getItem("digitalData") == "Offer Specific" && fromLocation.length > 2 && destinationLocation.length > 2) {
        setIsDisabled(false);
      }
            ...
            >
              <Stack>
                ...
                {localStorage.getItem("digitalData") ===
                  ...
                  <>
                    <Box sx={boxStyle}>
                      <Typography sx={{ ...fieldStyle, fontSize: "14px", fontWeight: "500" }}>Add Location</Typography>
                    </Box>
                    <br />
                    ...
                    >
                      <div sx={{ flexBasis: "50%" }}>
                        <Typography sx={CommonTextStyle}>From<span style={{ color: "red" }}> *</span></Typography>
                        <Box sx={{ position: "relative" }}>
                          <img style={{ position: "absolute", left: "10px", zIndex: "9", top: "20px" }} src={AirplaneIcon} />
                          <Input
                            disableUnderline
                            value={fromLocation}
                      ...
                      <div sx={{ flexBasis: "50%" }}>
                        <Typography sx={CommonTextStyle}>Destination<span style={{ color: "red" }}> *</span></Typography>
                        <Box sx={{ position: "relative" }}>
                          <img style={{ position: "absolute", left: "10px", zIndex: "9", top: "20px" }} src={AirplaneIcon} />
                          <Input
                            disableUnderline
                  ...
                  <>
                    <Box>
                      <Box sx={boxStyle}>
                        <Typography sx={{ ...fieldStyle, fontSize: "14px", fontWeight: "500" }}>How can it be redeemed by buyer ?<span style={{ color: "red" }}> *</span></Typography>
                      </Box>
                      <RadioGroup
                        row
                      ...
                      >
                        ...
                        <FormControlLabel
                          ...
                          style={{
                            color:
                              redeemedValue === "Both" ? "#445FD2" : "#ADB8CC",
                          }}
                        />
                      </RadioGroup>
                    </Box>
                ...
                <Box sx={boxStyle}>
                  <Typography sx={{ ...fieldStyle, fontSize: "16px", padding: "40px 0 0 0", fontWeight: "500" }}>Additional Cost</Typography>
                </Box>
                <OthercostsTemplate
                  append={(data, index) => {
                    if (index !== null) {
                    ...
                    <Box sx={boxStyle19}>
                      ...
                      >
                        {AllFeature?.map((el) => {
                          return (
                            <MenuItem
                              key={el.SampleAirlineFeature}
                              value={el.SampleAirlineFeature}
                              sx={CommonTextStyle}
                            >
                              <Typography sx={{
                                color: "#445fd2",
...
export default AirlineTicketsGeneralInformation;
