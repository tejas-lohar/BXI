...
export default function TextileGoLive() {
  ...
  const FetchProduct = async () => {
    await axios
      ...
      .then((response) => {
        ...
        console.log("response", response.data.GapBetweenDays);
      })
      .catch((error) => {
  ...
  const uploadData = async (event) => {
    ...
    if (Number(ListPeriod) >  Number(ProductData?.GapBetweenDays)) {
      // alert("Please select the list period less than 365 days");
      return toast.error(
        "Please select the list period less than " +  Number(ProductData?.GapBetweenDays)  + " Days",
        {
          position: "top-center",
          autoClose: 5000,
          hideProgressBar: false,
