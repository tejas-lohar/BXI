...
export default function HotelsGoLive() {
  ...
  const uploadData = async (event) => {
    setNextClicked(true);
    if(invalidFile || invalidFileFormat || invalidFileSize || !files ||
      !timeToListProduct.amount || timeToListProduct.amount === undefined ||
    ...
    await axios({
      ...
      .then((res) => {
        console.log(res);
        navigate("/home/hotels/voucherdesign/" + id);
      })
  ...
  return (
    <form>
      ...
      >
        ...
        {( (files?.length === 0 || !files) && nextClicked) ? <Typography
              sx={invalidFileStyle}
            >This field is required</Typography> :""}

        {( invalidFile && nextClicked && files && files?.length !== 0) ? <Typography
              sx={invalidFileStyle}
            >Invalid file</Typography> :""}

        {(invalidFileFormat && nextClicked && files && files?.length !== 0) ? <Typography
              sx={invalidFileStyle}
            >Invalid File Format (only JPEG, PNG, GIF, MP4, PDF file format are Supported)</Typography> :""}

        {(invalidFileSize && nextClicked && files && files?.length !== 0) ? <Typography
              sx={invalidFileStyle}
            >Invalid File Size (Maximum allowed file size is 5 MB)</Typography> :""}
        ...
        <Box sx={{ width: "90%", mx: "auto", mt: 2 }}>
          ...
          >
            ...
            {nextClicked && (!timeToListProduct.amount || timeToListProduct.amount === undefined ||
              timeToListProduct.amount === "")? (<Typography sx={invalidFileStyle}>This field is required</Typography>):""}
          </Box>
        </Box>
        <Box
          sx={{
