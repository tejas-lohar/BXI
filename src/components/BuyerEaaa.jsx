...
import { GetBuyerAllInvoices } from "../Hooks/GetBuyerAllInvoices";
...
function BuyerEaaa() {
  ...
  const {
    data: buyerAllInvoices,
    isLoading: loadingbuyerallinvoices,
    error,
    refetch: refetchbuyerallinvoices,
  } = GetBuyerAllInvoices(currentPage);

  const {
    mutate: buyerUploadPaymentDocs,
    isLoading: buyerUploadPaymentDocsLoading,
    error: buyerUploadPaymentDocsError,
    refetch: refetchbuyerUploadPaymentDocs,
  } = useBuyerUploadPaymentDocs(currentPage);
  ...
  const UploadDocs = async (invoiceId) => {
    buyerUploadPaymentDocs(
      ...
      {
        onSuccess: (data) => {
          console.log("ressubmitdoc", data.data);
  ...
  return (
    <Paper
      ...
      elevation={0}
    >
      <BreadCrumbHeader MainText={"Buyer Eaaa List"} />
      <Grid
        container
        sx={{
          background: "#FFFFFF",
        ...
        <Grid item xl={12} lg={12} md={12} sm={12} xs={12}>
          ...
          >
            ...
            <Grid item xl={1.5} lg={1.5} md={1.5} sm={1.5} xs={1.5}>
              <Typography align="center" sx={tableheading}>
                {/* <Typography align="left" sx={tableheading}> */}
                Order date
              </Typography>
            </Grid>
                          ...
                          >
                            <Typography
                              sx={{
                                ...
                                fontSize: "14px",
                                color:
                                  buyerEaaaAcceptedStatus === false
                                    ? "#FFB600"
                                    : buyerEaaaAcceptedStatus === true
                                    ? "#118A2C"
                                    : "#FF0000",
                            ...
                            >
                              {buyerEaaaAcceptedStatus ? "accepted" : "pending"}
                            </Typography>
                            {buyerEaaaAcceptedStatus === false ? (
                              <Box
                                component="img"
          ...
          {/* {Switchtab === "Purchased" ? (
            ...
            <React.Fragment>
              {purchaseOrderDataSellerAfterAcceptOrReject?.length > 0 &&
                purchaseOrderDataSellerAfterAcceptOrReject?.map((data, idx) => {
                  let totalPrice = 0;
                  data?.ProductData?.map((data) => {
                    totalPrice +=
                      data.DiscountedPrice *
                  ...
                  if (purchaseOrderLoadingSellerPendingAfterAcceptOrReject) {
                    return (
                      <Skeleton
                        variant="rectangular"
                        width={"100%"}
